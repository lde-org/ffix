...
test.it("cast with ctype passes through", function()
	...
	local p = c:cast(c:typeof("$ *", ct), v)
	test.equal(p.n, 7)
end)

test.it("typeof with dollar substitution builds pointer type", function()
	local c = ctx()
	c:cdef("typedef struct { float v; } Flt2;")
	local ct = c:typeof("Flt2")
	local ptr_ct = c:typeof("$ *", ct)
	local v = c:new("Flt2", { v = 1.5 })
	local p = ffi.cast(ptr_ct, v)
	test.equal(p.v, 1.5)
end)
